...
namespace eschool.Reports
{
	...
	/// </summary>
	public class WeightageReport : System.Web.UI.Page
	{
						...
						for(int p=0;p<count1;p++)
						{
							for(int q=0;q<count;q++)
							{
								...
								if(!(DropClass.SelectedItem .Text=="I"||DropClass .SelectedItem .Text=="II"||DropClass .SelectedItem .Text=="Nursery" ||DropClass .SelectedItem .Text=="LKG" ||DropClass .SelectedItem .Text=="UKG"))
								{
									...
									marks=System.Convert.ToDouble(Test1[p,q+2])+System.Convert.ToDouble(Assgn[p,q])+System.Convert.ToDouble(Test2[p,q]);
									gtotal[p]+=marks;
		...
		private void btnPrint_Click(object sender, System.EventArgs e)
		{
			try
			{
				if(View!=0)
				{
					...
					string path = home_drive + @"\Inetpub\wwwroot\eschool\Sysitem\eSchoolPrintService\WeightageReport.txt";
					...
					if(DropClass.SelectedIndex!=0 && DropSec.SelectedIndex!=0)
					{
						...
						sw.WriteLine(GenUtil.GetCenterAddr("WEIGHTAGE REPORT",desdes.Length));
						sw.WriteLine(GenUtil.GetCenterAddr("------------------",desdes.Length));
		...
		public void ConvertIntoExcel()
		{
			try
			{
				...
				string path = home_drive+@"\eSchool_ExcelFile\WeightageReport.xls";
